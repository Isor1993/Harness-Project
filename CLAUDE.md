...
Oben liegen nur diese Datei, `INDEX.md`, `PLAN.md` und `.claude/`.

**Die Wurzel dieses Repos ist der Harness selbst** — keine Weiterleitung,
keine Ebene darüber. Diese Datei lädt von selbst und trägt die Regeln;
deshalb gibt es seit 2.0.0 keinen Notkern mehr und keine zweite
`CLAUDE.md`.

Projekt-Code liegt **nicht** hier, sondern in seinem eigenen Repo, das
über `additionalDirectories` freigegeben wird. Ins Projekt wandert der
Harness nur als Auslieferung (`Kern/VERSIONIERUNG.md`).
...
- **Zeigen statt vorstellen lassen.** Nie „stell dir vor" — stattdessen
  ...
  in `Kern/WORKFLOW.md`.
- **In freigegebenen Unity-Projekten nie durchsuchen:** `Library/`,
