...
## Baustein 1 — Struktur begradigen ✅ **erledigt am 2026-08-23**

Ausgeführt als Harness **2.0.0**. Wurzel ist jetzt
`C:\Repos Isor\Harness Project\`. Abgleich nach `DOC_RULES.md` 11.3:
52 `.md`-Dateien und 8.634 nicht-leere Zeilen vorher wie nachher.
`pruefen.py` danach: 0 Funde in allen fünf Prüfungen.

**Vier Abweichungen vom Plan unten — alle beim Ausführen gefunden:**

1. **Handgriff 3 war schon erfüllt.** `.claude\` lag bereits in
   `Harness Project\`, also auf der Ebene, die Wurzel wurde.
2. **Handgriff 8 ist entfallen.** Weil der geöffnete Ordner derselbe
   blieb, sind Memory und Session-Historie gar nicht erst umgezogen.
   Das war der Grund, `Harness Project\` als Wurzel zu wählen und nicht
   einen kürzeren Namen.
3. **Drei Posten fehlten in Handgriff 1:** `UserSettings\`,
   `.gitattributes` und `Uni Coding Guidelines C# (SAE 12-2024).pdf`.
   Die PDF ist ein bitgleiches Duplikat von
   `C:\IsorBackup\01_Uni\_Regelwerk\Coding Guidelines C#.pdf`
   (beide 327.057 Bytes, gleicher SHA256) und wurde deshalb archiviert
   statt mitgenommen.
4. **Zwei Fallen, die der Plan nicht sah.** `.claude\` und `.git\` liegen
   jetzt *innerhalb* der Wurzel — beide Skripte hätten sie
   mitdurchlaufen und die fünf Arbeitskopien der Befehle als
   Harness-Dokumente gezählt. Dazu suchte `finde()` in `pruefen.py`
   zwei Ebenen nach oben nach `CLAUDE.md`, weil es sie dreimal gab.
   Beides ist behoben.

Archiv des Ballasts:
`C:\IsorBackup\99_Archiv\_Zu_Loeschen\2026-08-23_Harness_Umbau_Struktur\`
— darin auch die 1.843 MB `Library\`, die als Erstes geleert werden
können; Unity erzeugt sie neu.

**Offen geblieben:** die Auslieferung `Harness_2.0.0` (nach
`VERSIONIERUNG.md` fällig, sobald `X` sich ändert) und das Umbenennen
des GitHub-Repos — beides steht in `Kern/ROADMAP.md`.
